<?php
    ...
    class Data_Validate{
        ...
        function __construct($token, $tokenValidate, $requestMethod, $cred)
        {
            $this->token = $token;
        ...
        function data_Validate($getQuery, $getValue, $getType){
            $sql = new SQL_Query();
            $jwt = new JWT_Data();
            ...
            if($status == SUCCESS_RESPONSE){
                ...
                try{
                    if($_SERVER['REQUEST_METHOD'] === $this->requestMethod){
                        if($this->token === $this->tokenValidate){
                            ...
                            echo json_encode(array('Status'=>$status, 'Message'=>'Unautorized user'));  
                        }
                    }else{
