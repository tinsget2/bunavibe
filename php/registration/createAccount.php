<?php
    ...
    $alias = substr($email, 0, strpos($email,"@")).$time.rand(100,1000);

    $verifyPassword = rand(100000,1000000);

    $sql_Query = new Less_Sequre_Query('POST', 'Insert');

    $query = "INSERT INTO `userinfo`(`alies`, `email`,`password`,`verification`) VALUES (?, ?, ?, ?)";
    $value = array($alias, $email, $password, $verifyPassword);
    $type = 'ssss';

    $result = $sql_Query->less_Sequre($query, $value, $type);

    if(!($result == "") || !($result == null)){
        sendEmail($email, $verifyPassword, $result);        
    }
