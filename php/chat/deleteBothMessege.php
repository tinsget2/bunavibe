<?php
    ...
    $messageAlias = $input->messageAlias;
    $messageRecipent = $input->messageRecipent;

    $sql_Query = new Data_Validate($token, $createdToken, 'DELETE', 'Delete');

    $query = "DELETE FROM `message` WHERE `alies`=? AND ((`userOne`=? AND `userTwo`=?) OR (`userOne`=? AND `userTwo`=?))";
    $value = array($messageAlias, $alies, $messageRecipent, $messageRecipent, $alies);
    $type = 'sssss';
