<?php
    ...
    $messageAlies = substr($email, 0, strpos($email,"@")).$time.rand(100,1000);

    $sql_Query = new Data_Validate($token, $createdToken, 'POST', 'Insert');

    $query = "INSERT INTO `message`(`alies`, `userOne`, `userTwo`, `message`, `role`) VALUES (?,?,?,?,?), (?,?,?,?,?)";
    $value = array($messageAlies, $alies, $messageRecipent, $message, 1, $messageAlies, $messageRecipent, $alies, $message, 2);
    $type = 'ssssissssi';
