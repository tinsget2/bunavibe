<?php
    ...
    $aliasLikedUser = $input->aliasLikedUser; 

    $sql_Query = new Data_Validate($token, $createdToken, 'POST', 'Insert');

    $query = "INSERT INTO `matching`(`aliesUserOne`, `userOneAccept`, `aliesUserTwo`) VALUES (?, ?, ?) ON DUPLICATE KEY UPDATE `userOneAccept`=?";
    $value = array($alies, 1, $aliasLikedUser, 1);
    $type = 'sisi';
