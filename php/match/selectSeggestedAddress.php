<?php
    ...
    //Get Value
    $matchedAlias = $_GET['matchedAlias'];
    

    $sql_Query = new Data_Validate($token, $createdToken, 'GET', 'Select');

    $query = "SELECT `country`, `city`, `location` FROM `address` WHERE `alies`=? AND `privacy`=? AND `status`=? LIMIT ?";
    $value = array($matchedAlias, 1, 1, 10000);
    $type = 'siii';
