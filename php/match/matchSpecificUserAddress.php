...
//This php is for getting posted vacancies to the poster 
    ...
    $query = "SELECT `a`.`alies`, `a`.`country`, `a`.`city`, `a`.`location` FROM `address` `a` RIGHT JOIN `matching` `m` ON `a`.`alies`=`m`.`aliesUserOne` WHERE `alies`=? AND `a`.`privacy`=? AND `a`.`status`=? AND `m`.`userOneMatchStatus` = ? LIMIT ?";
