...
//This php is for getting posted vacancies to the poster 
    ...
    $query = "SELECT `i`.`alies`, `i`.`interest` FROM `interests` `i` RIGHT JOIN `matching` `m` ON `i`.`alies`=`m`.`aliesUserOne` WHERE `i`.`alies`=? AND `i`.`privacy`=? AND `i`.`status`=? AND `m`.`userOneMatchStatus` = ? LIMIT ?";
