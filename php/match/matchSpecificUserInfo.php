...
//This php is for getting posted vacancies to the poster 
    ...
    $query = "SELECT  `u`.`alies`, `u`.`email`, `u`.`fullName`, `u`.`phone`, `u`.`gender`, `u`.`InterestedIn`, `u`.`dateOfBirth`, `u`.`password`, `u`.`verification`, `u`.`about`, `u`.`onlineStatus` FROM `userinfo` `u` RIGHT JOIN `matching` `m` ON `u`.`alies`=`m`.`aliesUserOne` WHERE `u`.`alies` = ? AND (`u`.`accountType` = ? OR `u`.`accountType` = ?) AND `u`.`status` = ? AND `u`.`privacy` = ? AND `m`.`userOneMatchStatus` = ? LIMIT ?";
    $value = array($friendAlies, 1, 3, 1, 1, 1, 1);
    $type = 'siiiiii';
