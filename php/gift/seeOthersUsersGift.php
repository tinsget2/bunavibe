<?php
    ...
    $query = "SELECT `gt`.`perchaseNumber`, `gt`.`giftAlies`, `gt`.`giftOwner`, `gt`.`noOfGiftsSend`, `gt`.`noOfGiftsRecived`, `gt`.`totalGift`, `gt`.`approval`, `gt`.`checkOutAmount`, `gt`.`checkOutApproved`, `gp`.`giftType` FROM `gifttotal` `gt` LEFT JOIN `giftspackage` `gp` ON `gp`.`alias`=`gt`.`giftAlies` WHERE `gt`.`privacy` = ? AND `gt`.`status` = ? AND NOT `gt`.`giftOwner` = ? LIMIT ?";
    $value = array(1, 1, $alies, 1000);
    $type = 'iisi';
