<?php
    ...
    $query = "SELECT `gt`.`perchaseNumber`, `gt`.`giftAlies`, `gt`.`giftOwner`, `gt`.`noOfGiftsSend`, `gt`.`noOfGiftsRecived`, `gt`.`totalGift`, `gt`.`approval`, `gt`.`checkOutAmount`, `gt`.`checkOutApproved`, `gp`.`giftType`, `gp`.`unitPrice`,
    (`gp`.`unitPrice`*`gt`.`totalGift`) AS `totaleAmount` FROM `gifttotal` `gt` LEFT JOIN
    `giftspackage` `gp` ON `gp`.`alias`=`gt`.`giftAlies` WHERE `giftOwner` = ? LIMIT ?";
    $value = array($alies, 1000);
    $type = 'si';
