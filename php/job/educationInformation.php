<?php
    ...
    $query = "SELECT `e`.`alies`, `e`.`eduAlies`, `e`.`degreeType`, `e`.`graduatedIn`, `e`.`minor`, `e`.`major`, `e`.`graduatedFrom`, `e`.`description`, `e`.`startDate`, `e`.`endDate` FROM `education` `e` RIGHT JOIN `apply` `a` ON `a`.`alias`=`e`.`alies`
    WHERE `e`.`alies`=? AND `e`.`privacy`=? AND `e`.`status`=?";
    $value = array($userAlias, 1,1);
    $type = 'sii';
