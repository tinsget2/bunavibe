<?php
    ...
    $query = "SELECT `u`.`alies`, `u`.`email`, `u`.`fullName`, `u`.`phone`, `u`.`gender`, `u`.`dateOfBirth`, `u`.`about`, `a`.`jobAlies`, `c`.`path`, `c`.`name` FROM `apply` `a` LEFT JOIN `userinfo` `u` ON `a`.`alias` = `u`.`alies` AND `u`.`privacy`= ? AND `u`.`status` = ? LEFT JOIN `cv` `c` ON `c`.`alies`=`u`.`alies` AND `c`.`privacy` = ? AND `c`.`status` = ? WHERE `a`.`jobAlies` = ? LIMIT ?";
    $value = array(1, 1, 1, 1, $jobAlies, 1000);
    $type = 'iiiisi';
