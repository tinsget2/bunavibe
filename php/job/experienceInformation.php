<?php
    ...
    $query = "SELECT `e`.`alies`, `e`.`expAlies`, `e`.`companyName`, `e`.`position`, `e`.`startDate`, `e`.`endDate`, `e`.`description` FROM `experience` `e` RIGHT JOIN `apply` `a` ON `a`.`alias`=`e`.`alies` WHERE `e`.`privacy`=? AND `e`.`status`=? AND `e`.`alies`=?";
    $value = array(1,1,$userAlias);
    $type = 'iis';
